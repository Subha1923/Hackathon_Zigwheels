...
public class UsedCars extends BasePage {
	...
	public void clickSelectedCar() throws IOException, InterruptedException {
		javaScriptManager.highlightElement(driver, usedCarLocation);
	...
	public void clickPopularModels() throws IOException, InterruptedException {
		List<WebElement> ListOfModels = driver.findElements(By.xpath("//ul[@class=\"zw-sr-secLev usedCarMakeModelList popularModels ml-20 mt-10\"]/li/label"));
		boolean flag = true;
		int row = 1;
		excelUtility.setCellData("PopularModels", row, 0, "PopularModels");
		System.out.println("-----------------------------");
		for(WebElement model : ListOfModels) {
			javaScriptManager.scrollIntoView(driver, BrandAndModel);
			System.out.println(model.getText());
			excelUtility.setCellData("PopularModels", row, 0, model.getText());
			JavascriptExecutor jse = (JavascriptExecutor)driver;
			jse.executeScript("arguments[0].click();", model);
			if(flag) {
				new BaseClass().screenshot("PopularModelsList");					
				flag = false;
			}
			Thread.sleep(1000);
			row++;
